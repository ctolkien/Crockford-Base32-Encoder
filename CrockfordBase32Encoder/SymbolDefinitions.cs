...
namespace CrockfordBaseEncoder
{
    ...
    {
        private readonly List<SymbolDefinition> extraCheckDigits = new List<SymbolDefinition>();
        ...
        public IDictionary<int, char> ValueEncodings => this.ToDictionary(s => s.Value, s => s.EncodeSymbol);

        public IDictionary<int, char> CheckDigitEncodings => this
                    .Union(extraCheckDigits)
                    .ToDictionary(s => s.Value, s => s.EncodeSymbol);

        public IDictionary<char, int> ValueDecodings => this
                    .SelectMany(s => s.DecodeSymbols.Select(d => new { s.Value, DecodeSymbol = d }))
                    .ToDictionary(s => s.DecodeSymbol, s => s.Value);

        public IDictionary<char, int> CheckDigitDecodings => this
                    .Union(extraCheckDigits)
                    .SelectMany(s => s.DecodeSymbols.Select(d => new { s.Value, DecodeSymbol = d }))
                    .ToDictionary(s => s.DecodeSymbol, s => s.Value);
    }
